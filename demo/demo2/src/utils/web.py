...
def set_ssoparam():
    ...
    return cbc.encrypt(jwt.createJWT(payload=dict(app_name=app_name, app_id=app_id, app_secret=app_secret), expiredSeconds=300))


def set_sessionId(uid, seconds=43200):
    """设置cookie"""
    return cbc.encrypt(jwt.createJWT(payload=dict(uid=uid), expiredSeconds=seconds))
