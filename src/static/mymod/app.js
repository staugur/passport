...
layui.define(["passport", "table", "element", "form", "layer", "util"], function(exports){
    var passport = layui.passport, table = layui.table, element = layui.element, form = layui.form, layer = layui.layer, $ = layui.jquery, util = layui.util;
    //显示当前tab
    if(location.hash){
    ...
    table.on('tool(userAppTable)', function(obj){ //注：tool是工具条事件名，userAppTable是table原始容器的属性 lay-filter="对应的值"
        ...
        if(layEvent === 'detail'){ //查看
            ...
            layer.confirm('真的删除 '+ data.name+' 应用么？', {icon: 3, title:'温馨提示'}, function(index){
                layer.close(index);
                //向服务端发送删除指令
                ...
                }, {data: {"name": data.name}, method: "delete"});
            });
    ...
    form.on('submit(newApp)', function(data){
        ...
        }, {data: field, method: "post", beforeSend: function() {
            //禁用按钮防止重复提交
            $("#newAppSubmit").attr({ disabled: "disabled" });
