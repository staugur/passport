...
layui.define(["passport", "util", "layer", "laytpl"], function(exports){
    var passport = layui.passport, util = layui.util, layer = layui.layer, laytpl = layui.laytpl, $ = layui.jquery;
    laytpl.config({
        open: '<%',
        close: '%>'
    });
    ...
    minemsg.on('click', '.mine-msg li .fly-mark', function(){
        ...
        }, {data: {msgId: id}, method: "post", msgprefix: "标记消息失败："});
    });
    //删除一条消息
    minemsg.on('click', '.mine-msg li .fly-remove', function(){
        ...
        }, {data: {msgId: id}, method: "delete", msgprefix: "删除消息失败："});
    });
    //删除全部消息
    delAll.on('click', function(){
        var othis = $(this);
        layer.confirm('确定清空消息吗？<br>此操作将清空所有已读、未读消息！', {icon: 3, title:'温馨提示'}, function(index){
            ...
            }, {method: "delete", msgprefix: "清空消息失败："});
        });
    });
    //输出接口
