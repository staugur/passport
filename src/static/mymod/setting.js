...
layui.define(["passport", "element", "form", "layer", "laydate", "util", "upload", "flow"], function(exports){
        ...
        passport.ajax("/api/user/profile/?Action=profile", function(res) {
            ...
            if (res.code==0) {
                ...
                if (res.lock.domain_name) {
                    $("#domain_name").attr({ disabled: "disabled" });
                    $('#domain_name').addClass("layui-disabled");
                }
                //更新导航中昵称
                $('#nav_nickname').text(field.nick_name || '');
            } else {
                console.log('profile invalid', res.invalid);
                var tip = res.msg;
                var err = '';
                for (var index in res.invalid) {
                    err += profile[res.invalid[index]] + " ";
                }
                popup(tip+"："+err);
            }
        }, {data: field, method: "put", msgprefix: false, beforeSend: function() {
            //禁用按钮防止重复提交
    ...
    form.on('submit(password)', function(data){
        ...
        }, {data: field, method: 'put', msgprefix: '修改密码失败：', beforeSend: function() {
            //禁用按钮防止重复提交
    ...
    $("#sureCut").on("click",function () {
        if ($("#tailoringImg").attr("src") == null ){
            ...
            }, {data: {picStr: base64url.replace('data:image/png;base64,', '')}, beforeSend: function() {
                //禁用按钮防止重复提交
